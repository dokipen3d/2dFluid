...
inline int array2Dto1D(int x, int y, int dim) {
    return x + y * dim;
}
float ImplicitCircle(int x, int y, int radius) {
    float X = x * x;
    float Y = y * y;
...
inline float fastModf(float in, float& inpart) {
    // floor or int? floor because of negatives
    inpart = floor(in);
    return (in - inpart);
}

float sampleUExact(const vector<float>& uArray, int u, int v, int dim) {
    return (uArray[array2Dto1D(u, v, dim)] + uArray[array2Dto1D(u + 1, v, dim)]) / 2.0f;
}

float sampleVExact(const vector<float>& vArray, int u, int v, int dim) {
    return (vArray[array2Dto1D(u, v, dim)] + vArray[array2Dto1D(u, v + 1, dim)]) / 2.0f;
}

float sampleUAtFaceU(const vector<float>& uArray, int u, int v, int dim) {
    return uArray[array2Dto1D(u, v, dim)];
}

float sampleVAtFaceU(const vector<float>& vArray, int u, int v, int dim) {
    return (vArray[array2Dto1D(u, v - 1, dim)] + vArray[array2Dto1D(u + 1, v - 1, dim)] +
            vArray[array2Dto1D(u, v, dim)] + vArray[array2Dto1D(u + 1, v, dim)]) /
           4.0f;
}

float sampleUAtFaceV(const vector<float>& uArray, int u, int v, int dim) {
    return (uArray[array2Dto1D(u, v - 1, dim)] + uArray[array2Dto1D(u + 1, v - 1, dim)] +
            uArray[array2Dto1D(u, v, dim)] + uArray[array2Dto1D(u + 1, v, dim)]) /
           4.0f;
}

float sampleVAtFaceV(const vector<float>& vArray, int u, int v, int dim) {
    return vArray[array2Dto1D(u, v, dim)];
}

float sampleTrilinear(const vector<float>& arrayIn, float u, float v, int dim) {
    ...
    float tmp1{0.0f}, tmp2{0.0f}, tmp3{0.0f}, tmp4{0.0f}, tmp5{0.0f}, tmp12{0.0f}, tmp34{0.0f};
...
template <typename T> inline T when_eq(T x, T y) {
    return 1.0 - abs(sign(x - y));
}

template <typename T> inline T when_neq(T x, T y) {
    return abs(sign(x - y));
}

template <typename T> inline T when_gt(T x, T y) {
    return max(sign(x - y), (T)(0));
}

template <typename T> inline T when_lt(T x, T y) {
    return max(sign(y - x), (T)(0));
}

template <typename T> inline T when_ge(T x, T y) {
    return 1.0 - when_lt(x, y);
}

template <typename T> inline T when_le(T x, T y) {
    return 1.0 - when_gt(x, y);
}

template <typename T> inline T or_(T a, T b) {
    return min(a + b, 1.0f);
}

void simpleAdvect(const vArray& u, const vArray& v, vArray& densityWrite, const vArray& densityRead,
                  const int dimension) {
    int i;
    int j;
...
#pragma omp parallel for
    for (j = 1; j < dimension - 1; j++) {
        ...
        for (i = 1; i < dimension - 1; i++) {
            ...
            float finalOutside = or_(or_(or_(outsideleft, outsideright), outsidebottom), outsidetop);
            ...
            //      float densitySample =
            //        mix(sampleTrilinear(densityRead, uOffset, vOffset,
            //        dimension), 0.0f,
            //            finalOutside);
            float densitySample = sampleTrilinear(densityRead, uOffset, vOffset, dimension);
            densityWrite[array2Dto1D(i, j, dimension)] = densitySample;
        }
    }
}

void macCormackAdvect(const vArray& u, const vArray& v, vArray& densityWrite, vArray& tempDensity,
...
void bfecc(const vArray& u, const vArray& v, vArray& work1, vArray& work2, const vArray& phiN, const int dimension) {
    // Local notation
...
#pragma omp parallel for
    for (j = 1; j < dimension - 1; j++) {
        ...
        for (i = 01; i < dimension - 1; i++) {
            ...
            float densitySample = sampleTrilinear(phiHatN1, uOffset, vOffset, dimension);
...
#pragma omp parallel for
    for (j = 1; j < dimension - 1; j++) {
        for (i = 1; i < dimension - 1; i++) {
            ...
            float densitySample = sampleTrilinear(phiBarN, uOffset, vOffset, dimension);

            reuse[array2Dto1D(i, j, dimension)] = min(max(densitySample, phiMin), phiMax);
        }
    }
}

void bouyancy(vArray& u, vArray& v, const vArray& densityRead, const int dimension) {
    int i;
    int j;
...
#pragma omp parallel for
    for (j = 0; j < dimension; j++) {
        ...
        for (i = 0; i < dimension; i++) {
            ...
            v[array2Dto1D(i, j, dimension + 1)] += (densityRead[array2Dto1D(i, j, dimension)] / 2.0f) * mult;
        }
    }
#pragma omp barrier
...
#pragma omp parallel for
    for (j = 0; j < dimension; j++) {
        ...
        for (i = 0; i < dimension; i++) {
            ...
            v[array2Dto1D(i, j + 1, dimension + 1)] += (densityRead[array2Dto1D(i, j, dimension)] / 2.0f) * mult;
        }
    }
}

void calcDivergence(const vArray& u, const vArray& v, vArray& divergence, const int dimension) {
    int i;
    int j;
...
#pragma omp parallel for
    for (j = 1; j < dimension - 1; j++) {
        ...
        for (i = 1; i < dimension; i++) {
            // if we are on the bounary wall. not sure what to do to stop
            // wrapping around
            ...
            divergence[array2Dto1D(i, j, dimension)] =
                ((u[array2Dto1D(i + 1, j, dimension + 1)] - u[array2Dto1D(i, j, dimension + 1)])) +
                ((v[array2Dto1D(i, j + 1, dimension + 1)] - v[array2Dto1D(i, j, dimension + 1)])) +
                ((u[array2Dto1D(i, j, dimension + 1)] - 0.0f) * leftWallMask) - //  plus extra correction term ie
                                                                                //  bridson pg 49ed 1
                ((u[array2Dto1D(i + 1, j, dimension + 1)] - 0.0f) * rightWallMask) +
                ((v[array2Dto1D(i, j, dimension + 1)] - 0.0f) * bottomWallMask) -
                ((v[array2Dto1D(i, j + 1, dimension + 1)] - 0.0f) * topWallMask);
        }
    }
}

void pressureSolveJacobi(const vArray& divergence, vArray& pressureFrom, vArray& pressureTo, int iterations,
                         const int dimension) {
    int i;
    int j;
    int k;
...
#pragma omp parallel for
        for (j = 1; j < dimension - 1; j++) {
            ...
            for (i = 1; i < dimension - 1; i++) {
                ...
                // set pressure for the neighbours to be the same as current.
                // trick for reducing coefficient
                float PL = mix(pressureFrom[array2Dto1D(i - 1, j, dimension)], currentFrom, leftWallMask);
                float PR = mix(pressureFrom[array2Dto1D(i + 1, j, dimension)], currentFrom, rightWallMask);
                float PB = mix(pressureFrom[array2Dto1D(i, j - 1, dimension)], currentFrom, bottomWallMask);
                float PT = mix(pressureFrom[array2Dto1D(i, j + 1, dimension)], currentFrom, topWallMask);
...
void advectVelSimple(vArray& uFrom, vArray& vFrom, vArray& uTo, vArray& vTo, const int dimension) {
    int i;
    int j;
...
#pragma omp parallel for
    for (j = 1; j < dimension; j++) {
        for (i = 1; i < dimension; i++) {
            ...
            currentUVelU = std::min((float)(dimension), std::max(0.0f, currentUVelU));
            currentVVelU = std::min((float)(dimension), std::max(0.0f, currentVVelU));
            currentUVelV = std::min((float)(dimension), std::max(0.0f, currentUVelV));
            currentVVelV = std::min((float)(dimension), std::max(0.0f, currentVVelV));

            uTo[array2Dto1D(i, j, dimension)] = sampleTrilinear(uFrom, currentUVelU, currentVVelU, dimension);

            vTo[array2Dto1D(i, j, dimension)] = sampleTrilinear(vFrom, currentUVelV, currentVVelV, dimension);
        }
    }
}

void project(const vArray& pressure, vArray& vu, vArray& vv, int dimension) {
    int i, j = 0;
#pragma omp parallel for
    for (j = 1; j < dimension - 1; j++) {
        //#pragma omp simd safelen(32)
        for (i = 1; i < dimension - 1; i++) {
            vu[array2Dto1D(i, j, dimension + 1)] -= pressure[array2Dto1D(i, j, dimension)];
            vv[array2Dto1D(i, j, dimension + 1)] -= pressure[array2Dto1D(i, j, dimension)];
        }
    }
...
#pragma omp parallel for
    for (j = 1; j < dimension - 1; j++) {
        //#pragma omp simd safelen(32)
        for (i = 1; i < dimension - 1; i++) {
            vu[array2Dto1D(i + 1, j, dimension + 1)] += pressure[array2Dto1D(i, j, dimension)];
            vv[array2Dto1D(i, j + 1, dimension + 1)] += pressure[array2Dto1D(i, j, dimension)];
        }
    }

    // this goes from 0 to end because we want to set boundary values for all
    // vel
#pragma omp parallel for
    for (j = 0; j <= dimension; j++) {
        //#pragma omp simd safelen(32)
...
void emit(vArray& density, vArray& u, int dimension, double time) {
...
#pragma omp parallel for
    for (int j = 0; j < dimension; j++) {
        for (int i = 0; i < dimension; i++) {
            ...
            int insideBoundary = leftWallMask | rightWallMask | bottomWallMask | topWallMask;

            if (ImplicitCircle((i + 0.5f + sin(time * 0.1f) * 10) - (dimension / 2), (j + 0.5f) - (dimension / 6),
                               (dimension / 8)) < 0.5f) {
                density[array2Dto1D(i, j, dimension)] += mix(0.04, 0.0f, insideBoundary);
...
int main(int argc, char* argv[]) {
    ...
    alignas(32) vector<float> density(dimension * dimension), density2(dimension * dimension),
        tempDensity(dimension * dimension), divergence(dimension * dimension), pressure(dimension * dimension),
    ...
    for (int j = 0; j < MAC_DIM; j++) {
        for (int i = 0; i < MAC_DIM; i++) {
            //            u[array2Dto1D(i, j, MAC_DIM)] =
            //            sin(i*0.17f)*0.07f+0.1f;
    ...
    for (int j = 0; j < MAC_DIM; j++) {
        for (int i = 0; i < MAC_DIM; i++) {
            //            v[array2Dto1D(i, j, MAC_DIM)] =
            //            sin(i*0.15f)*0.05f+0.22f;
    ...
    if (SDL_Init(SDL_INIT_VIDEO) < 0) {
        ...
        window = SDL_CreateWindow("SDL Tutorial", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, screenDim,
                                  screenDim, SDL_WINDOW_OPENGL | SDL_WINDOW_BORDERLESS);
        if (window == NULL) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, dimension, dimension, 0, GL_RED, GL_FLOAT, density.data());
    ...
    while (eventLoop) {
        while (SDL_PollEvent(&keyevent)) // Poll our SDL key event for any keystrokes.
        {
            ...
            case SDL_KEYDOWN:
                ...
                case SDLK_ESCAPE: eventLoop = false; break;
                }
            }
        }
        // do stuff
